...
const txt = {
  fr: {
    ...
    keywords:
      "billet, musée, jeu vidéo, jeux, jeux vidéo, console, consoles, consoles de jeu vidéo, consoles de jeu vidéo portables, arcade, bornes d'arcade, ordinateur, ordinateurs, informatique, exposition, histoire, histoire du jeu vidéo",
  ...
  en: {
    ...
    keywords:
      'ticket, museum, video game, games, video games, console, consoles, video game consoles, portable video game consoles, arcade, arcade machines, computer, computers, information, history, video game history',
  ...
  return (
    <>
      ...
      >
        ...
        <div id="step-2">
          <h3 class="text-center" id="select-slot-title">
            {t().select_slot}
          </h3>
          <Slots
        ...
        <div id="step-3" class="mx-auto">
          <h3 class="text-center" id="select-price-title">
            {t().select_price}
          </h3>
          <p class="text-center text-sm text-text italic">
            {t().for_group}
            <A class="pl-2" href={`/${lang() as string}/contact`}>
              {t().contact}
            </A>
          </p>
          <Prices labelId="select-price-title" />
          <div class="mt-6">
            <h3 class="text-center" id="select-donation-title">
              {t().donation}
            </h3>
        ...
        <div id="step-4" class="max-w-sm mx-auto lg:self-start">
          <h3 class="text-center">{t().personal_infos}</h3>
          <PersonalInfos />
          <GiftCodes />
          <TicketResume />
          <div class="m-4 flex flex-col gap-4">
            <div class="grid grid-cols-[auto_1fr] items-center gap-2">
              <input
                ...
                checked={paymentCtrl.IHaveReadTheTermsAndConditions()}
                onChange={() =>
                  paymentCtrl.setIHaveReadTheTermsAndConditions(
                    !paymentCtrl.IHaveReadTheTermsAndConditions(),
                  )
                }
              />
            ...
            <a
              href={`/${lang() as string}/cgv`}
              target="_blank"
              rel="noopener noreferrer"
              class="text-center text-sm text-primary hover:text-primary inline-flex items-center gap-1 justify-center"
            >
          ...
          <PaymentBtn
            disabled={paymentCtrl.paymentButtonDisabled()}
            isLoading={paymentCtrl.isLoading()}
            onPayment={paymentCtrl.preparePayment}
          />
          <Show
            when={paymentCtrl.paymentButtonDisabled() && paymentCtrl.paymentButtonErrorMessage()}
          >
