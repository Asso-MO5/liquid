...
export const Page = () => {
  ...
  const regularSchedules = createMemo(() => {
    return schedules()
      .filter((schedule) => !schedule.is_exception && !schedule.is_closed)
                          ...
                          >
                            <div class="flex-1">
                              {(() => {
                                const { t } = translate({
                                  fr: {
                                    name: price.translations.fr?.name || '',
                                    description: price.translations.fr?.description || '',
                                  },
                                  en: {
                                    name: price.translations.en?.name || '',
                                    description: price.translations.en?.description || '',
                                  },
                                })
                                return (
                                  <>
                                    <h3 class=" text-xl mb-1 text-primary">{t().name}</h3>
                                    <Show when={t().description}>
                                      <p class="text-sm text-text italic">{t().description}</p>
                                    </Show>
                                  </>
                                )
                              })()}
                            ...
                            <div class="text-xl text-secondary">
                              <span class="text-text">{price.amount}</span>
