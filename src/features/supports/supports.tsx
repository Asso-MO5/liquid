          ...
          {(testimonial) => {
            const trans = translate({
              fr: {
                ...
                quote: testimonial.quote.fr,
                link: testimonial.link.fr,
              },
              en: {
                name: testimonial.name,
                image: testimonial.image,
                quote: testimonial.quote.en,
                link: testimonial.link.en,
              },
            })

            const nonReactiveIndex = supportsTestimonials.findIndex(
              (t) => testimonial.name === t.name,
            )
            ...
            return (
              <li
                ...
                class="hidden flex-col gap-4 items-center data-[visible=true]:flex md:flex-row data-[reverse=true]:md:flex-row-reverse"
              >
                <div class="flex md:justify-start justify-center items-center flex-col gap-3 flex-grow min-w-3xs">
                  ...
                  <a
                    ...
                    class="btn btn-sm mt-4 inline-block"
                    aria-label={`${t().ariaLabel}${trans.t().name}`}
                  >
                    {t().readMore}
                  </a>
                </div>
