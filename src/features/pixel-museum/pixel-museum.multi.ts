...
export const initMultiplayer = async ({ k, player, startPosition }: MultiplayerOptions) => {
  const room = gameRoom
  ...
  const initRemotePlayers = async () => {
    ...
    const createOrUpdateRemotePlayer = async (
      ...
      if (!remotePlayer?.exists() && !creatingPlayers.has(sessionId)) {
      ...
      if (playerState.flipX !== undefined) remotePlayer.flipX = playerState.flipX
      if (playerState.anim && remotePlayer.getCurAnim()?.name !== playerState.anim) {
        try {
    ...
    if (typeof players.onAdd === 'function' && typeof players.onRemove === 'function') {
      players.onAdd(async (playerState: Room['state']['players'], sessionId: string) => {
      ...
      players.onRemove((_playerState: Room['state']['players'], sessionId: string) => {
      ...
      players.forEach((playerState: Room['state']['players'], sessionId: string) => {
      ...
      k.onUpdate(() => {
        ...
        if (currentSize < lastPlayersSize) {
          ...
          players.forEach((_playerState: Room['state']['players'], sessionId: string) => {
  ...
  if (room.onStateChange) {
    room.onStateChange(() => {
      if (room.state?.players && remotePlayers.size === 0 && !isInitializing) {
        initRemotePlayers()
      }
    })
  }
