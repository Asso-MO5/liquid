...
export const giftCodesCtrl = () => {
  ...
  const addGiftCode = () => {
    if (
      ticketStore.gift_codes.length <
      ticketStore.tickets.filter((ticket) => ticket.amount > 0).length
    ) {
  ...
  const removeGiftCode = (index: number) => {
    setTicketStore('gift_codes', (codes) =>
      codes.filter((_, i) => i !== index).map((code, i) => ({ index: i, code: code.code })),
    )
    const input = document.querySelector(`#gift-code-${ticketStore.gift_codes.length - 1}`)
    if (input) {
      ;(input as HTMLInputElement).focus()
