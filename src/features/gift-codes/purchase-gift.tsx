    ...
    <section class="max-w-xl mx-auto flex flex-col gap-6 p-4">
      ...
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-3">
          ...
          <div class="flex flex-col gap-4">
            <div class="flex h-full text-xl items-center justify-center gap-2">
              <button
                onClick={() => ctrl.setQuantity(ctrl.quantity() - 1)}
                type="button"
              ...
              <button
                ...
                onClick={() => ctrl.setQuantity(ctrl.quantity() + 1)}
                disabled={ctrl.quantity() >= ctrl.maxQuantity}
              >
                {'\u002B'}
              </button>
            </div>
        ...
        <Show when={ctrl.validityDate()}>
          <p class="text-sm text-center text-accent">
            {t().validityDateMsg.replace('{date}', ctrl.validityDate())}
          </p>
        </Show>
        ...
        <div class="flex flex-col gap-2 mt-2 border-t border-primary pt-4 text-sm">
          <Show
            ...
            fallback={<p class="italic text-text/80">{t().unitPriceLoading}</p>}
          >
            <p>
            ...
            <Show when={ctrl.totalPrice() != null}>
              <p class="text-lg">
                ...
                <span class="font-bold text-secondary">{ctrl.totalPrice()?.toFixed(2)}€</span>
              </p>
            </Show>
          </Show>
        </div>
