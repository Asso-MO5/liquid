...
export const purchaseGiftCTRL = () => {
  ...
  const quantityValid = createMemo(() => quantity() >= 1 && quantity() <= maxQuantity)
  ...
  const fetchPrice = async () => {
    ...
    try {
      const response = await fetch(`${clientEnv.VITE_API_URL}/museum/settings/gift_code_price`, {
        headers: {
  ...
  const preparePurchase = async () => {
    ...
    try {
      ...
      const response = await fetch(`${clientEnv.VITE_API_URL}/museum/gift-codes/purchase`, {
        method: 'POST',
        headers: {
      ...
      try {
        window.localStorage.setItem('museum_gift_codes_checkout_id', data.checkout_id)
      } catch {
  ...
  const validityDate = createMemo(() => {
    ...
    return validityDate.toLocaleDateString(lang(), {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    })
