    ...
    <div class="flex flex-col gap-12 max-w-5xl mx-auto p-6">
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-12">
        <For each={images}>
          {(image) => {
            const { t } = translate({
              fr: { text: image.text.fr, alt: image.alt.fr },
              en: { text: image.text.en, alt: image.alt.en },
            })
            return (
              <li class="group relative overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 aspect-[4/3]">
                <img
                  src={image.src}
                  alt={t().alt}
                  ...
                  loading="lazy"
                />
                <p class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent md:opacity-0 group-focus:opacity-100 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4 text-white text-sm font-medium m-0">
                  {t().text}
                </p>
              </li>
            )
