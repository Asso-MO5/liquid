...
export const ticketResumeCTRL = () => {
  ...
  const getTicketCount = (ticketId: string) =>
    ticketStore.tickets.filter((t) => t.id === ticketId).length
  ...
  const getTicketPrice = (ticket: { amount: number }) =>
    ticketCreateCtrl.isHalfPrice() ? Math.floor(ticket.amount / 2) : ticket.amount
