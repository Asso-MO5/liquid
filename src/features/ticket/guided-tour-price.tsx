  ...
  return (
    <div class="grid grid-cols-[auto_1fr] items-center  justify-start gap-2 w-full max-w-sm my-4">
      <input
        ...
        id="guided-tour"
        class="w-4 h-4"
        aria-label={txt[lang() as keyof typeof txt].alt}
        checked={ticketStore.guided_tour}
        onChange={() => setTicketStore('guided_tour', !ticketStore.guided_tour)}
      />
      <label for="guided-tour" class="whitespace-nowrap">
        {txt[lang() as keyof typeof txt].title}{' '}
        <span class="text-secondary">{`(+${guidedTourPriceAmount}€)`}</span>
      </label>
    </div>
  )
}
